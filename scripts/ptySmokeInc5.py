#!/usr/bin/env python3
"""Increment 5 PTY smoke: the permission round trip now EXECUTES. After the
allow verdict the stub daemon emits a real tool_result frame whose output text
must surface on the collapsed tool card ("hello-from-stub"); the deny flow
stays output-free. Wire resolution is asserted from the request log.

Discipline (carried from Inc 1-4): stub UDS daemon, winsize ioctl before exec,
...
waits for repeated UI elements.
"""
                ...
                for line in fobj:
                    ...
                    if act == "v1/session/create":
                        reply({"id": rid, "status": "success",
                               "body": {"session_id": "stub-session-5"}})
                        ...
                        # v1/tool/resolve arrives on ANOTHER connection. On grant
                        # the tool_result frame rides back BEFORE the closing
                        # token, mirroring the real Inc 5 daemon.
                        ...
                        if granted:
                            ...
                            time.sleep(0.3)
                            reply({"type": "finished", "status": "completed",
                                   "sequence": 4})
                        else:
...
# ── Flow D1: ALLOW executes and shows the tool's real output ───────────────
def frames_log():
    try:
...
# ── Flow D2: DENY marks the card and never shows output ────────────────────
...
pump(1.2)   # settle: let the fresh dialog finish mounting before keystroke
...
while time.time() < deadline:
    ...
    if '"granted": false' in frames_log() or '"granted":false' in frames_log():
        ...
        break
if not wire_deny:
    # Diagnostic: show the LAST rendered screen (cursor-home delimited), not
    # the cumulative buffer, so the live state is readable.
    frames = clean(buf).split("\x1b[H")
    print("LAST SCREEN TAIL:\n" + (frames[-1] if frames else clean(buf))[-1200:])
print(("PASS" if wire_deny else "FAIL") + " deny-on-wire")
ok &= wire_deny
ok &= expect("denied-notice", "Denied bash")
seen_deny = clean(buf)
no_output = seen_deny.count("should-not-appear") == 0
print(("PASS" if no_output else "FAIL") + " no-output-on-deny")
ok &= no_output
