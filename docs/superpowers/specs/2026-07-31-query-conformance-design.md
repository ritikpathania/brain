...
| **Cross-Evaluator Isolation** | Evaluator execution MUST NOT modify any observable state affecting future results | All |
...
**Definition of Result Equality:** Two `QueryFacadeResult` instances are equal if and only if they possess:
1. Identical candidate `matches` vectors (matching `entity_id`, `active_facts_count`, `average_confidence`, `graph_metadata`, and `search_metadata`).
2. Identical candidate ordering.
3. Identical `total_matched` count.
4. Identical `snapshot_watermark`.
...
### 4. Exhaustive Pagination Algebra Contract
For any candidate set of size $T$:
- $\text{paginate}(\text{offset}=0, \text{limit}=N + M) = ([\text{matches}_{0..N+M}], T)$
- $\text{paginate}(\text{offset}=0, \text{limit}=N) \cup \text{paginate}(\text{offset}=N, \text{limit}=M) = ([\text{matches}_{0..N+M}], T)$
- $\text{paginate}(\text{offset}=0, \text{limit}=0) = ([], T)$
...
1. **Conformance Test Suite (`crates/brain-services/tests/query_conformance_tests.rs`)**:
   - `test_conformance_replay_equivalence_across_evaluators`: Validates batch replay vs incremental snapshot equivalence for `NeighborhoodEvaluator`, `SearchEvaluator`, and `HybridEvaluator` (including `snapshot_watermark` preservation).
   ...
   - `test_conformance_pagination_algebra`: Validates mathematical pagination equivalence (`limit=0`, `offset=total`, `offset>total`) and `total_matched` preservation.
