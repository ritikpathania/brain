...
- **Zero External Dependencies in Domain**: `brain-domain::projection` contains `ProjectionId`, `ProjectionVersion`, `Watermark`, `Checkpoint`, `ProjectionReducer` trait, and typed `ProjectionError` hierarchy with zero async runtimes, storage drivers, or database engines.
...
- **Replay Transparent Reducers**: Reducers process events via `apply_event(event)` regardless of whether events are live, replayed, or catch-up. Replay mode is transparently managed by the `ReplayEngine` via abstract event iterators.
- **Graceful Shutdown Sequence**: `Stop accepting events -> Finish current event -> Persist checkpoint -> Transition to Stopped`.
...
- **`instance.rs`**: `ProjectionInstance` container holding `reducer: Box<dyn ProjectionReducer>`, runtime state `lifecycle: ProjectionLifecycle`, `checkpoint: Checkpoint`, and `metrics: ProjectionMetrics`.
...
- **`replay.rs`**: `ReplayEngine` driving deterministic catch-up rebuilding from an abstract event iterator.
- **`scheduler.rs`**: `ProjectionScheduler` trait and `SequentialProjectionScheduler` implementation.
- **`runtime.rs`**: `ProjectionRuntime` facade orchestrating lifecycle transitions, event dispatch, catch-up replay, and graceful shutdown sequence.
...
2. **Replay & Invariant Tests (`crates/brain-services/tests/projection_replay_tests.rs`)**:
   ...
   - **Graceful Shutdown**: Verifies runtime shutdown stops event ingestion, flushes current event, and saves atomic checkpoint.