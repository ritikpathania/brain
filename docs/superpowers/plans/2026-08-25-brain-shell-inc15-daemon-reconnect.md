...
/** Wire a monitor to scripted probe results and an instant clock. */
...
describe('ConnectionMonitor', () => {
  it('walks attempts with growing delays and restores exactly once', async () => {
    const { h, monitor } = harness([false, false, true]);
    monitor.start();
    // Arming is synchronous: attempt 1 is already visible.
    ...
    // Delay precedes each probe: nextDelayMs(N) before probe N.
  ...
  it('stop() cancels the in-flight loop permanently', async () => {
    const { h, monitor } = harness([false, false, false]);
    monitor.start();
    await tick();
    expect(h.probeCalls).toBe(1);
    monitor.stop();
    for (let i = 0; i < 8; i++) await tick();
    // No further probes or transitions after stop.
    expect(h.probeCalls).toBe(1);
    expect(h.transitions).toEqual([{ status: 'reconnecting', attempt: 1 }]);
...
export class ConnectionMonitor {
  ...
  private async run(epoch: number): Promise<void> {
    ...
    for (;;) {
      ...
      await this.opts.delay(nextDelayMs(attempt));
      if (epoch !== this.epoch) return;
