...
fn test_temporal_state_record_insert_close_and_point_in_time_lookup() {
    ...
    assert_eq!(state.facts_at(&entity_id, t1).len(), 1);

    let t2 = Timestamp::now();
    state.close_interval(&fact_id, t2, FactLifecycle::Archived);
    assert!(state.active_facts(&entity_id).is_empty());
    assert!(!state.is_active(&fact_id));
    assert_eq!(state.record(&fact_id).unwrap().valid_until, Some(t2));
}
```
...
#[test]
fn test_temporal_state_reducer_event_application() {
    ...
    assert_eq!(reducer.state().active_facts(&entity_id).len(), 1);
    ...
    assert!(reducer.state().active_facts(&entity_id).is_empty());
}
```
