...
- Wire invariant: `sequence` numbers stay strictly consecutive across the WHOLE turn (gaps abort the shell stream with "Protocol violation"). `seq` starts at 0 on `stream_start` and increments once per subsequent frame; intermediate `Completed` chunks emit NO frame AND restore the slot the shared chunk-entry increment gave them (`seq -= 1`, discovered during execution — the legacy e2e proves the original daemon never duplicated slots); the terminal `stream_end` takes a fresh slot (`seq += 1` before write); the post-loop `finished` terminal frame (Invariant 3, handlers.rs ~2270) increments `seq` and stays untouched.
