...
Architecture Fitness Tests automatically enforce the 11 Constitutional Invariants ([CONSTITUTION.md](CONSTITUTION.md)) inside CI. Instead of relying on manual code review, fitness checks prevent boundary erosion, layer violations, and accidental mutation pathways.
...
| **4. Adapter Storage Isolation** | **Enforced** | `test_adapter_storage_isolation` | `cargo_metadata` dependency DAG check asserting adapter crates (`brain-daemon`, `apps/brain`) have zero direct dependencies on `brain-storage`. |
...
| **10. Primitive Composition** | **Enforced** | `test_primitive_composition` | Public API contract check asserting public exports match the 12 primitive types in [GLOSSARY.md](GLOSSARY.md). |
