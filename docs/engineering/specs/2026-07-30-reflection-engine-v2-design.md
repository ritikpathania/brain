...
Instead of mutating existing graph records in-place, Reflection Engine v2 relies on **Immutable Fact Versioning**, **Domain Value Objects**, and **Pure Observational Passes**. Reflection passes inspect read-only snapshots of the knowledge graph and propose declarative `RewritePlan`s. Plans from all active passes are merged by a `ConflictResolver`, validated by a `RewriteValidator`, and transactionally committed to the event log as an ordered sequence of immutable `FactEvent`s.
...
2. **Observational Pass Purity**: Reflection passes are pure, deterministic functions of `(Snapshot, Context)`. Passes inspect snapshots, perform analysis, and emit `RewritePlan` intent. Passes **must never** execute SQL, mutate storage, or emit events directly.
3. **Shared Frozen Snapshot Consistency**: All passes in a reflection cycle evaluate the exact same immutable snapshot of state. Passes never observe intermediate rewrites generated by other passes within the same cycle.
4. **Conflict Resolution & Ordered Operations**: Overlapping rewrites proposed by multiple passes are deterministically resolved by the `ConflictResolver`. Operations inside a `RewritePlan` form an ordered sequence executed sequentially inside a single atomic transaction.
5. **Single-Sided Supercedence**: Lineage links exist single-sided via `supersedes: Option<FactVersionId>`. Reverse lineage (`superseded_by`) is dynamically derived during query projection to prevent double-sided state drift.
6. **Derivable Lifecycle States**: `Historical` state is derived from non-null `valid_to` or active `superseded` links, avoiding duplicate mutable flags.
7. **Atomic Event Log Verification**: The event store WAL is the single source of truth. Read model projections are updated only after events are transactionally committed to the event log.
...
### 5.1 Reflection Context & Pass Execution Contracts
...
pub trait ReflectionPass: Send + Sync {
    ...
    ) -> Result<Option<ReflectionOutcome>, BrainServiceError>;
}
```

### 5.2 Pass Inventory & Reserved Inference Family
...
6. **`InferencePass` (Reserved Pipeline Family)**
   - *Design Reservation*: Separate pipeline family for multi-hop semantic deduction (e.g. `LivesIn(A, X)` $\land$ `HQ(Company, X)` $\implies$ `CommutesTo(A, X)`), ensuring reflection passes remain focused strictly on knowledge maintenance.

### 5.3 Conflict Resolution, Validation & Transactional Execution
...
When multiple passes propose operations targeting overlapping facts (e.g. Pass A superseding Fact X while Pass B merging Fact X), the `ConflictResolver` merges plans deterministically based on pass priority and cost rules:
...
impl ConflictResolver {
    pub fn resolve(
        ...
        snapshot: &dyn KnowledgeSnapshotView,
    ) -> Result<RewritePlan, BrainServiceError> {
        // Deduplicate operations, resolve supersede-vs-merge conflicts deterministically
    }
}
```

#### Transactional Executor & Atomic Commit
...
1. **Domain Value Objects**: `Timestamp`, `Confidence`, `EntityName`, `PredicateName`, `PredicateCardinality`, `LiteralValue`.
2. **Event Types**: `FactEvent::FactRecorded`, `FactEvent::FactSuperseded`, `FactEvent::FactArchived`.
3. **Snapshot Abstraction**: `KnowledgeSnapshotView` trait in `crates/brain-domain`.
4. **Rewrite Models**: `RewritePlan`, `RewriteOperation` (`RecordFact`, `SupersedeFact`, `MergeFacts`, `ArchiveFact`), `RewriteReason`.
5. **Validator**: `RewriteValidator` enforcing temporal and structural invariants.
6. **Conflict Resolver**: `ConflictResolver` merging multiple pass plans.
7. **Pass Interface**: `ReflectionPass` trait, `ReflectionContext`, `ReflectionOutcome`.
8. **Pass Registry & Topological Sort**: Engine pass DAG ordering by declared dependencies.
9. **`CanonicalizationPass`**: Casing/whitespace/alias normalization.
10. **`ContradictionPass`**: Exclusive predicate conflict detection & window closure.
11. **`DuplicateConsolidationPass`**: Entity and assertion deduplication into canonical facts.
12. **`StaleKnowledgePass`**: Temporal expiration analysis.
13. **`ConfidenceRecalculationPass`**: Lineage corroboration scoring.
14. **Rewrite Executor**: Transactional event-log appending & read-model updates.
15. **Verification Suite**: Determinism, replay invariant, and property-based tests.
