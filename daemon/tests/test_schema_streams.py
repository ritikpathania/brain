...
def test_stream_start_success(stream_validator):
    event = {"type": "stream_start", "streamId": "stream-123"}
...
def test_stream_start_with_metadata_and_extensions(stream_validator):
    event = {
        "type": "stream_start",
        "streamId": "stream-123",
        "metadata": {"client_timestamp": 123456789, "latency_ms": 1.5},
        "ext": {"custom_theme": "dark"},
    }
    stream_validator.validate(event)
...
def test_stream_end_success(stream_validator):
    event = {"type": "stream_end", "streamId": "stream-123", "sequence": 3}
    stream_validator.validate(event)


def test_stream_cancelled_success(stream_validator):
    event = {"type": "stream_cancelled", "streamId": "stream-123", "sequence": 3}
    stream_validator.validate(event)
...
def test_stream_progress_missing_required(stream_validator):
    ...
    event = {"type": "stream_progress", "streamId": "stream-123", "sequence": 1}
    with pytest.raises(ValidationError):
        stream_validator.validate(event)
...
def test_stream_invalid_type_enum(stream_validator):
    ...
    event = {"type": "stream_unknown", "streamId": "stream-123"}
    with pytest.raises(ValidationError):
        stream_validator.validate(event)
