    ...
    for ent in concept_entities:
        ...
        if overlapping:
            ...
            for sub_start, sub_end in sub_spans:
                ...
                strip_start = sub_start + (
                    len(content[sub_start:sub_end])
                    - len(content[sub_start:sub_end].lstrip())
                )
...
def extract_semantic_nodes(json_input: str) -> str:
    ...
    prop_noun_pattern = re.compile(r"\b[A-Z][a-zA-Z0-9_]*(?:\s+[A-Z][a-zA-Z0-9_]*)*\b")
    ...
    relation_patterns = [
        (
            "uses",
            re.compile(
                r"\b(uses|use|using|utilizes|utilize|utilizing|employs|employ|employing)\b",
                re.IGNORECASE,
            ),
        ),
        (
            "develops",
            re.compile(
                r"\b(develops|develop|developing|developed|built|builds|building|created|creates|creating)\b",
                re.IGNORECASE,
            ),
        ),
        (
    ...
    passive_relation_patterns = [
        ...
        (
            "develops",
            re.compile(r"\b(developed by|built by|created by)\b", re.IGNORECASE),
        ),
    ...
    for raw_node in raw_nodes:
        ...
        for match in class_pattern.finditer(content):
            ...
            class_id, class_label, _class_type = normalize_entity(classification)
        ...
        raw_entities = _split_overlapping_proper_nouns(
            raw_entities, content, stop_words
        )
        ...
        sorted_entities = sorted(
            raw_entities,
            key=lambda x: (
                -(x["match_end"] - x["match_start"]),
                TYPE_PRECEDENCE.get(x["type"], 99),
            ),
        )
        ...
        for ent in sorted_entities:
            ...
            for kept in resolved_entities:
                if max(ent["match_start"], kept["match_start"]) < min(
                    ent["match_end"], kept["match_end"]
                ):
                    overlap = True
                    break
            if not overlap:
        ...
        for ent in resolved_entities:
            ...
            if ent_id not in extracted_nodes:
                ...
                if TYPE_PRECEDENCE.get(ent["type"], 99) < TYPE_PRECEDENCE.get(
                    existing["type"], 99
                ):
                    ...
                    extracted_edges.append(
                        {
                            ...
                            if tech == "sqlite"
                                ...
                                "postgresql"
                                if tech in ("postgres", "postgresql")
                                else tech
                            ),
                            "relation": "configures",
                        }
