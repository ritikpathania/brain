...
def run_lifecycle_session(target_dir, key_sequence, cols=80, rows=24, initial_env=None, timeout=8.0):
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            ...
            if master_fd in r:
                ...
                if len(raw) > 0 and sent_input and time.time() - last_data > 1.0:
                    break
...
if __name__ == "__main__":
    ...
    if action == "compact_command":
        # Run /compact
        display = run_lifecycle_session(target_dir, ["/compact\n", 0.5], cols, rows)
        for i, l in enumerate(display): print(f"[{i:02d}] {l}")
    elif action == "doctor_command":
        # Run /doctor
        display = run_lifecycle_session(target_dir, ["/doctor\n", 0.5], cols, rows)
        for i, l in enumerate(display): print(f"[{i:02d}] {l}")
    elif action == "status_command":
        # Run /status
        display = run_lifecycle_session(target_dir, ["/status\n", 0.5], cols, rows)
        for i, l in enumerate(display): print(f"[{i:02d}] {l}")
