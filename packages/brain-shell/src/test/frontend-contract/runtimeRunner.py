...
def run_runtime_session(target_dir, key_sequence, cols=80, rows=24, initial_env=None, timeout=8.0):
...
if __name__ == "__main__":
    ...
    if action == "streaming_turn":
        display = run_runtime_session(target_dir, ["hello world\n", 0.5], cols, rows)
        for i, l in enumerate(display): print(f"[{i:02d}] {l}")
    elif action == "abort_ctrl_c":
        # Type input, submit, then send Ctrl+C (\x03) after 100ms
        display = run_runtime_session(target_dir, ["hello world\n", 0.1, "\x03", 0.3], cols, rows)
        for i, l in enumerate(display): print(f"[{i:02d}] {l}")
