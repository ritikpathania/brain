...
import { logForDebugging } from '../../vendor/claude/utils/debug.js'
import type { Diff, FlickerReason, Frame } from '../../vendor/claude/ink/frame.js'
import type { Point } from '../../vendor/claude/ink/layout/geometry.js'
import {
  type Cell,
  CellWidth,
  cellAt,
  ...
  shiftRows,
  visibleCellAtIndex,
} from '../../vendor/claude/ink/screen.js'
import {
  CURSOR_HOME,
  ...
  RESET_SCROLL_REGION,
  setScrollRegion,
} from '../../vendor/claude/ink/termio/csi.js'
import { LINK_END, link as oscLink } from '../../vendor/claude/ink/termio/osc.js'
...
export class LogUpdate {
    ...
    for (let y = 0; y < screen.height; y++) {
      ...
      for (let x = 0; x < screen.width; x++) {
        ...
        if (cell && cell.width !== CellWidth.SpacerTail) {
          ...
          let cellStyles = this.options.stylePool.get(cell.styleId)
          if (cell.char === ' ') {
            cellStyles = cellStyles.filter(c => c.endCode !== '\x1b[39m')
          }
  ...
  render(
    ...
    diffEach(prev.screen, next.screen, (x, y, removed, added) => {
      ...
      // If the cell outside the viewport range has changed, in altScreen or shrinking
      // mode we need a full reset. In normal steady-state, offscreen cells are skipped.
      if (y < viewportY) {
