...
function postprocessTokens(blocks: Block[], theme: Theme): Block[] {
  const result: Block[] = [];
  for (let i = 0; i < blocks.length; i++) {
    const [style, text] = blocks[i]!;
    if (text === 'console' && i + 1 < blocks.length && blocks[i + 1]![1].startsWith('.')) {
      result.push([{ foreground: theme.scopes['variable'] ?? theme.foreground, background: style.background }, text]);
      continue;
    }
    if (
      text.startsWith('.') &&
      (i + 1 < blocks.length && blocks[i + 1]![1].startsWith('(') || text.includes('('))
    ) {
      const match = text.match(/^(\.)([a-zA-Z0-9_$]+)(.*)$/);
      if (match) {
        const [, dot, fnName, rest] = match;
        result.push([{ foreground: theme.foreground, background: style.background }, dot!]);
        result.push([
          {
            foreground: theme.scopes['title.function'] ?? theme.scopes['built_in'] ?? theme.foreground,
            background: style.background,
          },
          fnName!,
        ]);
        if (rest) {
          result.push([{ foreground: theme.foreground, background: style.background }, rest]);
        }
        continue;
      }
    }
...
function highlightLine(
  ...
  return postprocessTokens(blocks, theme)
}
