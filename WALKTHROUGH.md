...
Python plugins dropped into `~/.brain/plugins/` must export a `register_plugins()` function:
...
3. **Execute Unit Tests**:
   ...
   ```
4. **Execute CLI Correctness Tests & Snapshots**:
   ```bash
   cd cli && bun test
   ```
5. **Execute TUI Render Benchmarks**:
   ```bash
   # Run deterministic rendering profile benchmark
   cd cli && bun run benchmark:render

   # Compare current profiling results to baseline
   cd cli && bun run benchmark:compare

   # Display chronological average rendering trends over the last 5 runs
