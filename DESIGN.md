---
version: beta
name: Brain-Design-System
description: >
  Design system specification for the Brain Terminal User Interface
  (TUI). The system renders rich interactive interfaces inside standard terminal
  emulators using Rust, Ratatui, and Crossterm. Color output supports truecolor
...
Brain is a terminal-based engineering assistant that provides a stateful, agentic interaction model directly in the user's terminal. The TUI runs entirely within terminal constraints using **Rust + Ratatui** (immediate-mode library) and **Crossterm** (terminal interface backend).
...
- **Block-fill progress** — `█` (filled) and `░` (empty) with progress limits to prevent backwards movement.
...
- **No true z-index stacking.** Ratatui draws widgets sequentially. Modals work by rendering at the top of the draw hierarchy and capturing keyboard focus — they do not "float" over other content.
...
- **Spinner component is monolithic.** The `spinner.rs` module handles multiple responsibilities (animation, tool progress, agent status, cost). This is identified technical debt.
...
Colors are resolved at render time using the `Theme` structure defined in [theme.rs](file:///Users/ritikpathania/Developer/PyCharm/brain/crates/brain-tui/src/ui/theme.rs). Every custom widget or styled string maps to semantic roles instead of hardcoding colors. The active theme implements dark, light, daltonized, and ANSI-16 fallback specifications.
...
The `ThemePicker` dialog shows all themes. Dismissing without selecting restores the original. Each option displays a **color swatch row** (small `█` blocks) showing the theme's key colors.
...
Controlled via the `wrap` prop on styled text:
...
### Character-Cell Grid
TUI spacing is measured in **character cells** (columns × rows), not pixels. Ratatui widgets use integer character-cell units.
...
### Layout Engine
Ratatui resolves widgets dynamically during frames:
- `Layout` grids divide the terminal area into sub-regions.
- `Constraint` maps sizes proportionally (Percentage, Max, Min).
- Widgets (Header, Chat list, Sidebar) render directly inside designated areas.

### Visual Dividers
- Horizontal lines are drawn using `─` (box-drawing horizontal) via the `Divider` widget.
- Dividers default to the `subtle` color and fill the available width.
- Optional centered titles break the line: `──── Title ────`

### Responsive Behavior

Terminals do not support CSS media queries. Responsiveness is handled by the immediate-mode event loop recalculating constraints on terminal resize events (`SIGWINCH` / `crossterm::event::Event::Resize`).
...
The design system layout guidelines apply to Ratatui custom widgets implemented under `crates/brain-tui/src/ui/`. While the legacy implementation used React primitives, the Rust widgets draw the same tokens:

### Text / Span
Formatted strings mapped onto Crossterm buffers. Resolves theme keys to raw colors. Supports styling attributes such as bold, dim, color, background, and underline.

### Layout / Block
Ratatui block drawing area. Resolves theme keys for borders and background colors.
...
- Do use theme tokens everywhere. Always map text styles onto `Theme` values.
- Do design for terminal resizing. Use flexible layouts that degrade cleanly when columns < 80.
- Do show activity during async operations. Always display animated spinner indicators.
- Don't use pixel-based attributes. Spacing and dimensions are character-cell based.
- Don't assume a specific background color. Keep the base floor transparent.
- Don't assume mouse support. Keyboard shortcuts (Vim navigation keys, arrows, Enter) are primary.
