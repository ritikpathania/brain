//! Append-only ArtifactStore maintaining artifacts and directed provenance graph edges.

use brain_domain::{
    ArtifactId, ArtifactView, DomainError, ExecutionArtifact, PlanStepId, ProvenanceEdge,
    ProvenanceEdgeId,
};
...
pub struct ArtifactStore {
    artifacts: HashMap<ArtifactId, ExecutionArtifact>,
    step_to_artifact: HashMap<PlanStepId, ArtifactId>,
    edges: HashMap<ProvenanceEdgeId, ProvenanceEdge>,
    // Adjacency maps for fast query resolution
    // outgoing: from (parent) -> list of edges where 'from' is origin
    outgoing: HashMap<ArtifactId, Vec<ProvenanceEdgeId>>,
    // incoming: to (child) -> list of edges where 'to' is target
    incoming: HashMap<ArtifactId, Vec<ProvenanceEdgeId>>,
}
...
impl ArtifactStore {
    ...
    pub fn insert(&mut self, artifact: ExecutionArtifact) -> Result<ArtifactId, DomainError> {
        let id = artifact.id;
    ...
    pub fn get(&self, id: ArtifactId) -> Option<ArtifactView<'_>> {
    ...
    pub fn parents(&self, id: ArtifactId) -> Vec<ArtifactId> {
    ...
    pub fn children(&self, id: ArtifactId) -> Vec<ArtifactId> {
