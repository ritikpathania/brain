...
fn random_event<R: Rng>(rng: &mut R) -> IngestionEvent {
    ...
    match rng.gen_range(0..13) {
        ...
        5 => IngestionEvent::TerminalCommand {
            ...
            exit_code: if rng.gen() {
                Some(rng.gen_range(-1..128))
            } else {
                None
            },
            stdout_summary: if rng.gen() {
                Some(random_string(rng, 50))
            } else {
                None
            },
            metadata,
        ...
        10 => {
            ...
            if rng.gen() {
                caps.push(Capability::ConversationMessages);
            }
            if rng.gen() {
                caps.push(Capability::ConversationTools);
            }
            if rng.gen() {
                caps.push(Capability::WorkspaceGit);
            }
            if rng.gen() {
                caps.push(Capability::WorkspaceFiles);
            }
            if rng.gen() {
                caps.push(Capability::WorkspaceTerminal);
            }
            if rng.gen() {
                caps.push(Capability::WorkspaceDiagnostics);
            }
            if rng.gen() {
                caps.push(Capability::Replay);
            }
            if rng.gen() {
                caps.push(Capability::Batching);
            }
...
fn test_protocol_fuzzing() {
    ...
    for _ in 0..100 {
        ...
        let json_str =
            brain_integrations::to_canonical_json(&envelope).expect("Fuzzer failed to serialize");

        // 2. Validate against JSON schema
        let json_val: serde_json::Value =
            serde_json::from_str(&json_str).expect("Fuzzer failed to parse JSON string");
        ...
        let deserialized: IngestionEnvelope =
            serde_json::from_str(&json_str).expect("Fuzzer failed to deserialize");
