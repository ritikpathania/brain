...
fn create_test_graph() -> (KnowledgeGraph, NodeId, NodeId, NodeId) {
    ...
    graph.add_node(Node::new(
        node_c,
        "Compiler Optimization".to_string(),
        NodeType::Concept,
    ));
...
fn test_retrieval_planning_and_optimization() {
    ...
    assert!(
        physical_plan.cost.total_cost() > 0.0,
        "Cost estimation must be calculated"
    );
...
fn test_sources_and_fusion() {
    ...
    let context =
        RetrievalExecutionContext::new(SnapshotId::new(1), &graph, &registry, &analytics, None);
...
fn test_retrieval_invariants_and_explanations() {
    ...
    assert_eq!(
        graph.nodes.len(),
        graph_before.nodes.len(),
        "Retrieval execution must never mutate the underlying graph"
    );
    assert_eq!(
        graph.edges.len(),
    ...
    assert_eq!(
        result1.candidates, result2.candidates,
        "Retrieval candidate rankings must be deterministic"
    );
    assert_eq!(
        result1.explanations, result2.explanations,
        "Retrieval explanations must be deterministic"
    );
    ...
    let explanation_a = result1
        .explanations
        .get(&node_a)
        .expect("Explanation for Node A missing");
    let has_semantic = explanation_a
        .evidence_list
        .iter()
        .any(|e| matches!(e, Evidence::SemanticMatch { .. }));
    assert!(has_semantic, "Node A should have semantic match evidence");

    let explanation_b = result1
        .explanations
        .get(&node_b)
        .expect("Explanation for Node B missing");
    let has_traversal = explanation_b
        .evidence_list
        .iter()
        .any(|e| matches!(e, Evidence::GraphTraversal { .. }));
...
fn test_declarative_query_dsl() {
    ...
    if let LogicalStep::ExpandNeighbors { policy, .. } = &logical_plan.steps[2] {
        ...
        assert!(policy
            .criteria
            .contains(&StoppingCriterion::MaxVisitedNodes(10)));
        assert!(policy
            .criteria
            .contains(&StoppingCriterion::MinConfidence(0.8)));
...
fn test_query_compilation_determinism_and_synonyms() {
    ...
    assert_eq!(
        res_lower.canonical_query, res_mixed.canonical_query,
        "Compilation must produce identical normalized queries"
    );
    assert_eq!(
        res_lower.canonical_query, res_upper.canonical_query,
        "Compilation must produce identical normalized queries"
    );

    // Assert synonym resolution
    assert_eq!(
        res_lower.canonical_query.semantic_query, "postgresql",
        "Common synonym postgres must compile to postgresql"
    );
    assert!(res_lower
        .metadata
        .passes_executed
        .contains(&"semantic_rewriter".to_string()));
    assert!(res_lower
        .metadata
        .diagnostics
        .iter()
        .any(|d| d.code == DiagnosticCode::AliasExpanded
            && d.severity == Severity::Info
            && d.code.as_str() == "CMP-100"
            && d.origin_pass == Some("semantic_rewriter".to_string())));
    ...
    for (s1, s2) in plan1.steps.iter().zip(plan2.steps.iter()) {
        match (s1, s2) {
            (
                LogicalStep::VectorRetrieve { query: q1 },
                LogicalStep::VectorRetrieve { query: q2 },
            ) => {
                assert_eq!(q1, q2);
            }
            (
                LogicalStep::KeywordRetrieve { query: q1 },
                LogicalStep::KeywordRetrieve { query: q2 },
            ) => {
                assert_eq!(q1, q2);
            }
            _ => {}
...
fn test_constant_folding_depth_zero() {
    ...
    assert!(res
        ...
        .diagnostics
        .iter()
        .any(|d| d.code == DiagnosticCode::ConstantFolded
            && d.severity == Severity::Warning
            && d.code.as_str() == "CMP-200"
            && d.origin_pass == Some("constant_folder".to_string())));
...
fn test_compiler_invalid_pass_configuration() {
    ...
    use crate::retrieval::planner::{ConstantFolder, LexicalNormalizer, SemanticRewriter};
    ...
    assert!(matches!(
        bad_order,
        Err(CompilerBuildError::InvalidPassOrdering { .. })
    ));
    ...
    assert!(matches!(
        duplicate_pass,
        Err(CompilerBuildError::DuplicatePass(..))
    ));

    // 3. Check missing Lexical phase error
    let missing_lexical = QueryCompiler::new(vec![Box::new(SemanticRewriter)]);
    assert!(matches!(
        missing_lexical,
        Err(CompilerBuildError::MissingRequiredPhase(
            CompilerPhase::Lexical
        ))
    ));

    // 4. Check missing Semantic phase error
    let missing_semantic = QueryCompiler::new(vec![Box::new(LexicalNormalizer)]);
    assert!(matches!(
        missing_semantic,
        Err(CompilerBuildError::MissingRequiredPhase(
...
fn test_event_driven_streaming_retrieval() {
    ...
    assert_eq!(
        batch_result.candidates, stream_result.candidates,
        "Batch and stream results must be identical"
    );

    // Telemetry and lifecycle validations:
    // Check Single Completion
    let completed_events: Vec<&RetrievalEvent> = sink
        .events()
        .iter()
    ...
    assert_eq!(
        ...
        1,
        "Completed event must be emitted exactly once"
    );
    ...
    for event in sink.events() {
        ...
        match event {
            RetrievalEvent::StageStarted { stage } => {
                ...
                assert!(
                    active_stages.is_empty(),
                    "Stage started before previous stage completed: {:?}",
                    stage
                );
            ...
            RetrievalEvent::StageCompleted { stage } => {
                let last_active = active_stages
                    .pop()
                    .expect("StageCompleted emitted but no stage active");
                assert_eq!(last_active, *stage, "StageCompleted mismatch");
            }
            RetrievalEvent::Completed { .. } => {
                assert!(
                    active_stages.is_empty(),
                    "Completed event emitted while stages are still active: {:?}",
                    active_stages
                );
                seen_completed = true;
            }
            RetrievalEvent::CandidateFound(_c) => {
                ...
                assert!(
                    !active_stages.is_empty(),
                    "CandidateFound emitted outside of active stage"
                );
                ...
                assert!(
                    ...
                    "CandidateFound emitted in non-retrieval stage: {:?}",
                    current_stage
                );
            }
            RetrievalEvent::ExplanationUpdated { .. } => {
                assert!(
                    !active_stages.is_empty(),
                    "ExplanationUpdated emitted outside of active stage"
                );
            }
        }
    }
