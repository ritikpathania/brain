...
fn test_graph_adjacency_state_insert_lookup_and_prune() {
    ...
    assert_eq!(state.neighbors_out(&source), std::slice::from_ref(&edge_id));
    assert_eq!(state.neighbors_in(&target), std::slice::from_ref(&edge_id));
    assert_eq!(state.degree(&source).out_degree, 1);
    assert_eq!(state.degree(&target).in_degree, 1);

    // Verify degree consistency invariant
    assert_eq!(
        state.degree(&source).out_degree,
        state.neighbors_out(&source).len()
    );
    assert_eq!(
        state.degree(&target).in_degree,
        state.neighbors_in(&target).len()
    );
