//! Declarative Knowledge Evolution models: EvolutionPlanId, EvolutionActionId, DomainEntityId, EvolutionAction, and EvolutionPlan.
...
/// Strongly-typed identifier for an evolution action.
#[derive(
    Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, serde::Serialize, serde::Deserialize,
)]
...
/// Capability-oriented action proposed to evolve system memory or knowledge graph state.
/// Invariant: EvolutionAction describes intent declaratively; it does not mutate memory directly.
...
impl EvolutionPlan {
    /// Instantiates a new `EvolutionPlan`.
    pub fn new(execution_id: ExecutionId, actions: Vec<EvolutionAction>) -> Self {
        Self {
