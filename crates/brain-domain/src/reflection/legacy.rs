//! Legacy reflection models maintained for backwards-compatibility.
...
/// Category classification for legacy reflection findings.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize)]
pub enum LegacyFindingKind {
...
impl std::fmt::Display for LegacyFindingKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl ReflectionFinding {
    ...
    pub fn kind(&self) -> LegacyFindingKind {
        match self {
            Self::DuplicateFound { .. } => LegacyFindingKind::Duplicate,
            Self::ContradictionFound { .. } => LegacyFindingKind::Contradiction,
            Self::LinkSuggested { .. } => LegacyFindingKind::LinkSuggestion,
        }
    }
...
pub struct ReflectionRecommendation {
    ...
    pub finding_kind: LegacyFindingKind,
    /// Confidence score between 0.0 and 1.0.
    pub confidence: f64,
