...
use crate::state::{InspectorSession, InspectorLoadState};

/// Stateless drawing function for the interactive Knowledge Inspector widget.
pub fn draw(f: &mut Frame<'_>, area: Rect, active: &InspectorSession, theme: &Theme, has_focus: bool, is_pinned: bool) {
    ...
    match &active.load_state {
        InspectorLoadState::Loading => {
            ...
            lines.push(Line::from(vec![
                Span::styled("  ⏳  Loading graph entity details...", Style::default().fg(Color::Cyan).add_modifier(Modifier::ITALIC))
            ]));
        }
        InspectorLoadState::Error(err) => {
            lines.push(Line::from(""));
            lines.push(Line::from(vec![
                Span::styled("  ⚠  Error fetching entity:", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD))
            ]));
            lines.push(Line::from(vec![
        ...
        InspectorLoadState::Loaded(model) => {
            ...
            let mut entity_spans = vec![
                ...
                Span::styled(&model.entity.label, Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
            ];
            if is_pinned {
                entity_spans.push(Span::styled(" [PINNED]", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)));
            }
            ...
            lines.push(Line::from(vec![
                Span::styled("Relationships & Adjacency", Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
            ]));
            if model.relationships.is_empty() {
                lines.push(Line::from(vec![
                    Span::styled("  (No connections recorded)", Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC))
                ]));
            } else {
                for (idx, rel) in model.relationships.iter().enumerate() {
                    ...
                    let direction_symbol = if rel.direction == "outgoing" { "→" } else { "←" };
                    
                    lines.push(Line::from(vec![
                        Span::styled(prefix, if is_selected { Style::default().fg(Color::LightBlue) } else { Style::default().fg(Color::DarkGray) }),
                        Span::styled(format!("{} [{}] ", direction_symbol, rel.direction), style.fg(Color::Yellow)),
            ...
            // 2. Metadata Section
            lines.push(Line::from(vec![
                Span::styled("Properties & Metadata", Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
            ]));
            ...
            if let Some(ref explanation) = model.retrieval_explanation {
                lines.push(Line::from(vec![
                    Span::styled("Retrieval Explanation", Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
                ]));
                lines.push(Line::from(vec![
                    Span::styled("  Score: ", Style::default().fg(Color::Gray)),
                    Span::styled(format!("{:.4}", explanation.score), Style::default().fg(Color::Green)),
                    Span::styled(format!(" (raw: {:.4})", explanation.raw_score), Style::default().fg(Color::DarkGray)),
                ]));
                if explanation.semantic_distance > 0.0 {
                    lines.push(Line::from(vec![
                        Span::styled("  Semantic Distance: ", Style::default().fg(Color::Gray)),
                        Span::styled(format!("{:.4}", explanation.semantic_distance), Style::default().fg(Color::Yellow)),
                    ]));
                }
                if !explanation.keyword_boosts.is_empty() {
                    lines.push(Line::from(vec![
                        Span::styled("  Matched Terms:     ", Style::default().fg(Color::Gray)),
                        Span::styled(explanation.keyword_boosts.join(", "), Style::default().fg(Color::Magenta)),
                    ]));
                }
                lines.push(Line::from(vec![
            ...
            lines.push(Line::from(vec![
                Span::styled("Provenance & Source History", Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
            ]));
            lines.push(Line::from(vec![
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(&model.provenance.location, Style::default().fg(Color::Yellow)),
            ]));
            if let Some(text_span) = model.provenance.extra_info.get("text_span") {
                lines.push(Line::from(vec![
                    Span::styled("  Text Span: ", Style::default().fg(Color::Gray)),
                    Span::styled(format!("\"{}\"", text_span), Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC)),
                ]));
            }
            lines.push(Line::from("─".repeat(max_width)));
            ...
            lines.push(Line::from(vec![
                Span::styled("Recent Activity Logs", Style::default().fg(theme.accent.fg.unwrap_or(Color::Cyan)).add_modifier(Modifier::BOLD)),
            ]));
            for log in &model.recent_activity {
                lines.push(Line::from(vec![
                    Span::styled(format!("  [{}] ", log.action), Style::default().fg(Color::Yellow)),
