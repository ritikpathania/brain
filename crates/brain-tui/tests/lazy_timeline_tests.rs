...
use brain_tui::ui::command::tool::{ToolExecution, ToolExecutionStatus, ToolLogEntry, ToolCallId, ToolId};
...
fn test_lazy_timeline_rendering() {
    ...
    let tool_call = ToolExecution {
        ...
        status: ToolExecutionStatus::Completed { result: "Success".to_string() },
        logs: vec![
            ToolLogEntry { message: "Searching for rust...".to_string(), timestamp: std::time::SystemTime::now() },
            ToolLogEntry { message: "Found 10 results".to_string(), timestamp: std::time::SystemTime::now() },
        ],
