...
use brain_tui::ui::layout::{LayoutEngine, StatusBarMeasure, DialogMeasure, CellWidth};

#[test]
fn test_status_bar_geometry_happy_path() {
    let area = Rect::new(0, 0, 30, 1);
    let measure = StatusBarMeasure { title_width: CellWidth(8), show_spinner: true };
...
fn test_status_bar_geometry_boundaries() {
    ...
    let measure = StatusBarMeasure { title_width: CellWidth(10), show_spinner: false };
    let geometry = LayoutEngine::status_bar(area, &measure);
...
fn test_dialog_geometry_happy_path() {
    ...
    let measure = DialogMeasure { button_widths: &button_widths };
