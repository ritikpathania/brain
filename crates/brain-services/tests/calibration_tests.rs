use brain_domain::retrieval::{
    CostHeuristics, EstimatedCost, HeuristicMetadata, HeuristicWeights, LogicalRetrievalPlan,
    LogicalStep, PlanOptimizer, PlanningMetadata, RetrievalExecutionReport, RuntimeMetadata,
};
...
fn make_report(
    ...
    RetrievalExecutionReport {
        ...
        runtime: RuntimeMetadata {
            ...
            candidates_produced: if vector_active || keyword_active {
                100
            } else {
                0
            },
...
fn test_calibration_confidence_gating() {
    ...
    assert!(
        active.weights.vector_weight > 10.0,
        "Vector weight should increase toward observed latency"
    );
    assert_eq!(
        calibrator.history().len(),
        1,
        "Should record 1 snapshot in history"
    );
...
fn test_calibration_purity_invariant() {
    ...
    assert!(
        !updated,
        "Result cache hits must be ignored to prevent cost deflation"
    );
    assert_eq!(calibrator.active_heuristics().metadata.version, 1);
}
...
fn test_calibration_snapshot_history_cap() {
    ...
    assert!(
        history.len() <= 10,
        "History ring buffer must cap at 10 items"
    );
}
...
fn test_calibration_tolerance_gating() {
    ...
    assert!(
        !updated,
        "Should not update if change is below 1e-5 tolerance"
    );
    assert_eq!(calibrator.active_heuristics().metadata.version, 1);
}
...
fn test_calibration_monotonic_convergence() {
    ...
    for _ in 0..15 {
        ...
        assert!(
            diff < last_diff,
            "Difference to target latency must converge monotonically"
        );
        last_diff = diff;
    }
    ...
    assert!(
        last_diff < 500.0,
        "Weight should have converged close to the target of 5000.0"
    );
}
...
fn test_calibration_plan_selection_adaptation() {
    ...
    let logical_plan = LogicalRetrievalPlan {
        steps: vec![
            LogicalStep::VectorRetrieve {
                query: "Rust".to_string(),
            },
            LogicalStep::KeywordRetrieve {
                query: "Rust".to_string(),
            },
        ],
    };
    ...
    assert_eq!(
        physical_plan2.cost.keyword_cost,
        physical_plan1.cost.keyword_cost
    );
}
