...
fn test_end_to_end_coordinator_orchestration_pipeline() {
    ...
    let enq_ev = CoordinatorEvent::External(Box::new(ExternalEvent::TaskEnqueued {
        task_id: t1,
    ...
    match enq_ev {
        CoordinatorEvent::External(ext) => match *ext {
            ExternalEvent::TaskEnqueued {
                task_id,
                execution_id,
                job_id,
                priority,
    ...
    assert!(failure_det
        .check_health("worker-1".to_string(), 1002)
        .is_none());
}
