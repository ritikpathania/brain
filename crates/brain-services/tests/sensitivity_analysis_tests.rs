...
use brain_services::eval_harness::{
    run_sensitivity_analysis, EvaluationSession, FeatureProvider, FtsRetriever, GroundTruthCorpus,
    HybridRetriever, QueryCorpus, RankingDecay, RankingWeights, SemanticRetriever,
...
fn test_sensitivity_analysis_execution_and_reporting() {
    ...
    fs::write(
        base_path.join("controlled_sensitivity_report.md"),
        &report_content,
    )
    .unwrap();
}
