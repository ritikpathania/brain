use brain_domain::retrieval::{
    CancellationChecker, CanonicalQuery, CompletionReason, ExecutionPolicy, NeverCancelled,
    NormalizedTieBreakerRanking, PhysicalRetrievalPlan, PhysicalStep, ReciprocalRankFusion,
    RecordingSink, RetrievalEvent, RetrievalExecutionContext, RetrievalExecutor, SpeculationPlan,
    SpeculationStrategy,
};
use brain_domain::{
...
fn test_speculation_transparency_and_hit() {
    ...
    let seq_executor = RetrievalExecutor::new(
        &context,
        ReciprocalRankFusion::default(),
        NormalizedTieBreakerRanking,
    )
    ...
    let spec_hit_executor = RetrievalExecutor::new(
        &context,
        ReciprocalRankFusion::default(),
        NormalizedTieBreakerRanking,
    )
    .with_policy(ExecutionPolicy::Speculative)
    .with_speculation_strategy(Box::new(DummyStrategy {
        seeds: vec![node_a],
    }));
    ...
    assert_eq!(
        hit_result.report.runtime.candidates_produced,
        seq_result.report.runtime.candidates_produced
    );
    assert_eq!(
        hit_result.report.runtime.expansions_performed,
        seq_result.report.runtime.expansions_performed
    );
...
fn test_speculation_miss_false_positive() {
    ...
    let spec_miss_executor = RetrievalExecutor::new(
        &context,
        ReciprocalRankFusion::default(),
        NormalizedTieBreakerRanking,
    )
    ...
    let miss_result =
        spec_miss_executor.execute_stream(plan.clone(), &mut miss_sink, &NeverCancelled);
    ...
    assert_eq!(
        miss_result.report.runtime.candidates_produced,
        seq_result.report.runtime.candidates_produced
    );
    assert_eq!(
        miss_result.report.runtime.expansions_performed,
        seq_result.report.runtime.expansions_performed
    );
...
fn test_speculative_cancellation_gating() {
    ...
    let spec_executor = RetrievalExecutor::new(
        &context,
        ReciprocalRankFusion::default(),
        NormalizedTieBreakerRanking,
    )
