...
#[test]
fn test_correlation_analysis_pearson_and_spearman() {
    ...
    for i in 0..8 {
        assert!((pearson_report.matrix.values[i][i] - 1.0).abs() < 1e-9);
        for j in 0..8 {
    ...
    for i in 0..8 {
        assert!((spearman_report.matrix.values[i][i] - 1.0).abs() < 1e-9);
        for j in 0..8 {
