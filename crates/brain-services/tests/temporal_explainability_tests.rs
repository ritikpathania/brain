...
fn test_temporal_explainability_and_invariants() {
    ...
    let (res_16, explanations_16) = service
        .retrieve_temporal(&session_id, "AlphaNode", 10, &query_16)
        .unwrap();
    assert!(!res_16.is_empty());

    // Check that explanations contain TemporalVisibility for the active edges
    let evidences_a = explanations_16
        .get(&node_a)
        .expect("AlphaNode should have explanations");

    let visibility_count = evidences_a
        .iter()
        .filter(|ev| matches!(ev, Evidence::TemporalVisibility { .. }))
        .count();
    // Both edges (A -> B and A -> C) are active at T=16, so there should be exactly 2 visibility items
    assert_eq!(
        visibility_count, 2,
        "Should emit exactly 2 visibility evidence items"
    );

    // Assert that the contents of the TemporalVisibility evidence match the edge and query parameters
    let mut evidences_a = explanations_16
        .get(&node_a)
        .expect("AlphaNode should have explanations")
        .clone();
    ...
    if let Evidence::TemporalVisibility {
        observed_at,
        validity_intervals,
        query_time,
        visibility_mode,
    ...
    let (res_decay, explanations_decay) = service
        .retrieve_temporal(&session_id, "AlphaNode", 10, &query_decay)
        .unwrap();
    assert!(!res_decay.is_empty());

    let evidences_decay = explanations_decay
        .get(&node_a)
        .expect("AlphaNode should have explanations");

    // Check that we got exactly one RecencyDecay evidence item (since policy is active)
    let decay_count = evidences_decay
        .iter()
        .filter(|ev| matches!(ev, Evidence::RecencyDecay { .. }))
        .count();
    assert_eq!(
        decay_count, 1,
        "Should emit exactly 1 recency decay evidence item"
    );

    // Extract decay details and check correctness/reproducibility
    if let Evidence::RecencyDecay {
        policy,
        observed_at,
        reference_time,
        elapsed_seconds,
        decay_factor,
    } = &evidences_decay[evidences_decay.len() - 1]
    {
    ...
    for _ in 0..5 {
        let (_, explanations_consec) = service
            .retrieve_temporal(&session_id, "AlphaNode", 10, &query_decay)
            .unwrap();
        let evidences_consec = explanations_consec.get(&node_a).unwrap();
        assert_eq!(
            evidences_decay, evidences_consec,
            "Evidence lists must be identical across consecutive executions"
        );
    }
    ...
    let (res_none, _) = service
        .retrieve_temporal(&session_id, "AlphaNode", 10, &query_16)
        .unwrap();

    // Retrieve again with the same parameters
    let (res_none_again, _) = service
        .retrieve_temporal(&session_id, "AlphaNode", 10, &query_16)
        .unwrap();
