...
#[test]
fn test_pruning_experiments_controlled_corpus() {
    ...
    // 3. Populate database programmatically (First pass: nodes, embeddings, events)
    ...
    // Verify outcomes are sorted descending by degradation (largest negative composite delta first, meaning largest loss value first)
    for i in 0..report.outcomes.len() - 1 {
        let loss_curr = report.outcomes[i].loss();
        let loss_next = report.outcomes[i + 1].loss();
        assert!(
            loss_curr >= loss_next,
            "Outcomes are not sorted descending by degradation: index {} has loss {}, index {} has loss {}",
            i,
            loss_curr,
            i + 1,
            loss_next
    ...
    // Write Report
    let report_md = PruningReportWriter::write_report(&report);
    let base_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/evaluation");
    fs::create_dir_all(&base_path).unwrap();
    fs::write(base_path.join("controlled_pruning_report.md"), &report_md).unwrap();
}
