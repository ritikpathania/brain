...
use brain_services::retrieval::eval_harness::{
    ...
    ScoreRanker,
    cv::{
        FoldAssigner, CrossValidationRunner, EvaluationMetric,
        compute_distribution,
    },
    models::{
        LambdaMartTrainingConfig, TrainingDataset,
    },
};
...
fn test_fold_assigner_deterministic_and_integrity_invariants() {
    ...
    for (f_idx, fold) in folds_1.iter().enumerate() {
        ...
        for q in &val_set {
            assert!(!train_set.contains(q), "Query {} found in both train and validation of fold {}", q, f_idx);
        }

        // every query in val_queries must appear exactly once across all validation folds
        for q in fold.val_queries.iter() {
            assert!(seen_queries.insert(q.clone()), "Query {} appears in multiple validation folds", q);
    ...
    for q in &query_ids {
        assert!(union_val_queries.contains(q), "Query {} is missing from validation folds", q);
    }
}
...
fn test_5_fold_cross_validation_evaluation() {
    ...
    for fold in &folds {
        ...
        let baseline_candidates = CalibrationEngine::run_calibration(&train_session, options, objective);
        ...
        let (lin_ndcg, lin_mrr, lin_recall, lin_comp) = evaluate_fold_metrics_for_model(&session, &linear_ranker, &val_set);
        ...
        let (log_ndcg, log_mrr, log_recall, log_comp) = evaluate_fold_metrics_for_model(&session, &logistic_model, &val_set);
    ...
    let lm_summary_comp = lm_cv.summary.distributions.get(&EvaluationMetric::Composite).unwrap();
    let lm_summary_ndcg = lm_cv.summary.distributions.get(&EvaluationMetric::NdcgAt5).unwrap();
    let lm_summary_mrr = lm_cv.summary.distributions.get(&EvaluationMetric::Mrr).unwrap();
    let lm_summary_recall = lm_cv.summary.distributions.get(&EvaluationMetric::RecallAt5).unwrap();
    ...
    for fold in &folds {
        md.push_str(&format!(
            "| {} | {} | {} |\n",
            fold.fold_idx, fold.train_queries.len(), fold.val_queries.len()
        ));
    }
    ...
    for fold in &folds {
        md.push_str(&format!(
            ...
            fold.fold_idx, fold.val_queries.join(", ")
        ));
    }
    ...
    md.push_str(&format!(
        ...
        linear_summary_comp.mean, linear_summary_comp.std_dev, linear_summary_comp.min, linear_summary_comp.max
    ));
    md.push_str(&format!(
        "| | Logistic Regression | {:.4} | {:.4} | {:.4} | {:.4} |\n",
        logistic_summary_comp.mean, logistic_summary_comp.std_dev, logistic_summary_comp.min, logistic_summary_comp.max
    ));
    md.push_str(&format!(
        "| | LambdaMART | {:.4} | {:.4} | {:.4} | {:.4} |\n",
    ...
    md.push_str(&format!(
        ...
        linear_summary_ndcg.mean, linear_summary_ndcg.std_dev, linear_summary_ndcg.min, linear_summary_ndcg.max
    ));
    md.push_str(&format!(
        "| | Logistic Regression | {:.4} | {:.4} | {:.4} | {:.4} |\n",
        logistic_summary_ndcg.mean, logistic_summary_ndcg.std_dev, logistic_summary_ndcg.min, logistic_summary_ndcg.max
    ));
    md.push_str(&format!(
        "| | LambdaMART | {:.4} | {:.4} | {:.4} | {:.4} |\n",
    ...
    md.push_str(&format!(
        ...
        linear_summary_mrr.mean, linear_summary_mrr.std_dev, linear_summary_mrr.min, linear_summary_mrr.max
    ));
    md.push_str(&format!(
        "| | Logistic Regression | {:.4} | {:.4} | {:.4} | {:.4} |\n",
        logistic_summary_mrr.mean, logistic_summary_mrr.std_dev, logistic_summary_mrr.min, logistic_summary_mrr.max
    ));
    md.push_str(&format!(
        "| | LambdaMART | {:.4} | {:.4} | {:.4} | {:.4} |\n",
    ...
    md.push_str(&format!(
        ...
        linear_summary_recall.mean, linear_summary_recall.std_dev, linear_summary_recall.min, linear_summary_recall.max
    ));
    md.push_str(&format!(
        "| | Logistic Regression | {:.4} | {:.4} | {:.4} | {:.4} |\n",
        logistic_summary_recall.mean, logistic_summary_recall.std_dev, logistic_summary_recall.min, logistic_summary_recall.max
    ));
    md.push_str(&format!(
        "| | LambdaMART | {:.4} | {:.4} | {:.4} | {:.4} |\n",
        lm_summary_recall.mean, lm_summary_recall.std_dev, lm_summary_recall.min, lm_summary_recall.max
    ));
