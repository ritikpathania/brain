use crate::query::dto::{JobDetails, JobSummary, SearchSummary, SessionDetails, SessionSummary};
...
pub trait JobQueryService: Send + Sync {
    ...
    fn list_jobs(&self, query: JobQuery)
        -> Result<Vec<JobSummary>, brain_core::errors::BrainError>;
    /// Retrieves full details of a specific job by its ID.
    fn get_job(
        &self,
        id: &brain_domain::jobs::JobId,
    ) -> Result<Option<JobDetails>, brain_core::errors::BrainError>;
}
...
pub trait SessionQueryService: Send + Sync {
    ...
    fn list_sessions(
        &self,
        query: SessionQuery,
    ) -> Result<Vec<SessionSummary>, brain_core::errors::BrainError>;
    /// Retrieves full details of a specific session (including message logs) by its ID.
    fn get_session(
        &self,
        id: &brain_domain::SessionId,
    ) -> Result<Option<SessionDetails>, brain_core::errors::BrainError>;
}
...
pub trait SearchQueryService: Send + Sync {
    ...
    fn search(
        &self,
        query: SearchQuery,
    ) -> Result<Vec<SearchSummary>, brain_core::errors::BrainError>;
}
